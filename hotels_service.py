...
class HotelsService(object):
    ...
    def standardize(self, hotels: list[Hotel]) -> list[Hotel]:
        ...
        for hotel in hotels:
            ...
            hotel.description = self.__standardize_description__(
                hotel.description)
    ...
    def merge_and_save(self, hotels: list[Hotel]):
        ...
        for hotel in hotels:
            ...
            merged_hotels[key] = Hotel(
                ...
                location=self.__merge_location__(existing_hotel.location,
                                                 hotel.location),
                amenities=self.__merge_amendities__(existing_hotel.amenities,
                                                    hotel.amenities),
                images=self.__merge_images__(existing_hotel.images,
                                             hotel.images),
                booking_conditions=self.__merge_booking_conditions__(
                    existing_hotel.booking_conditions,
                    hotel.booking_conditions))
    ...
    def find(self, hotel_ids, destination_ids) -> list[Hotel]:
        if hotel_ids == 'none' and destination_ids == 'none':
            return self.hotels
        hotel_ids = set(
            hotel_ids.split(',')) if hotel_ids != 'none' else self.hotel_ids
        destination_ids = set(destination_ids.split(
            ',')) if destination_ids != 'none' else self.destination_ids
        ...
        filtered = sorted(
            (hotel for hotel in self.hotels if hotel.id in hotel_ids
    ...
    def __standarize_amenities__(self, amenities: Amenities) -> Amenities:
        general = amenities.general if isinstance(amenities.general,
                                                  list) else []
    ...
    def __standarize_images__(self, images: Images) -> Images:
        ...
        amenities = images.amenities if isinstance(images.amenities,
                                                   list) else []
        return Images(
            rooms=rooms,
            site=site,
    ...
    def __standardize_booking_conditions__(
            self, booking_conditions: list[str]) -> list[str]:
        booking_conditions = booking_conditions if isinstance(
            booking_conditions, list) else []
        return [s.strip() for s in booking_conditions]
    ...
    def __merge_location__(self, location1: Location,
    ...
    def __merge_amendities__(self, am1: Amenities,
                             am2: Amenities) -> Amenities:
        return Amenities(
    ...
    def __merge_images__(self, images1: Images, images2: Images) -> Images:
        return Images(
            rooms=list(
                {img.link: img
                 for img in images1.rooms + images2.rooms}.values()),
            site=list({img.link: img
                       for img in images1.site + images2.site}.values()),
            amenities=list({
                img.link: img
                for img in images1.amenities + images2.amenities
            }.values()),
        )

    def __merge_booking_conditions__(self, conditions_1: list[str],
                                   conditions_2: list[str]) -> list[str]:
